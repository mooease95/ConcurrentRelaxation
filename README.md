...
This program has a built in function of f(x,y) = (x^2) + (y^2), which is used to generate and fill parameterised-sized 2d array, where x is the first dimension of the array and y is the second. It then creates a copy of the array, and fills all non-border elements with random numbers. The randomised copy of the array is solved sequentially and concurrently and the results are output in the console.
